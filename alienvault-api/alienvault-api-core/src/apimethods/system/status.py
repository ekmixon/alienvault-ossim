...
def system_all_info(system_id, no_cache=False):
    ...
    if success:
        return ans_system_all_info(system_ip)
    else:
...
def disk_usage(system_id):
    ...
    """
    (success, system_ip) = get_system_ip_from_system_id(system_id)
    return ans_disk_usage(system_ip) if success else APIResult(success, system_ip)
...
def cpu(system_id):
    ...
    """
    (success, system_ip) = get_system_ip_from_system_id(system_id)
    return ans_cpu(system_ip) if success else APIResult(success, system_ip)
