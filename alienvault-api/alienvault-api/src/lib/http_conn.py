...
class HttpConn(object):
    ...
    def connect(self,username, password):
        try:
            self.username = username
            self.password = password
            self.conn = httplib.HTTPConnection(host = self.host,port=self.port)
        except Exception,e:
...
if __name__ == "__main__":
    a = HttpConn("admin","alien4ever","127.0.0.1","40009")
    print a.do_request("http://127.0.0.1:40009/server/status", "")    